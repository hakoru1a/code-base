...
namespace Generate.Application.Features.Products.Queries.GetProducts
{
    public class GetProductsQueryHandler : IRequestHandler<GetProductsQuery, List<ProductResponseDto>>
    {
        ...
        {
            IQueryable<Generate.Domain.Products.Product> query = _productRepository.FindAll().Include(p => p.Category);
        ...
        private IQueryable<Generate.Domain.Products.Product> ApplyPolicyFilters(
            IQueryable<Generate.Domain.Products.Product> query,
