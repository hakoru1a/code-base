...
namespace Generate.Infrastructure
{
    public static class ConfigureServices
    {
        ...
        {
            var migrationsAssembly = typeof(GenerateContext).Assembly.FullName!;

            switch (provider)
            {
                case global::Infrastructure.DatabaseProviders.MySqlDatabaseProvider mysqlProvider:
                    mysqlProvider.ConfigureDbContext<GenerateContext>(services, connectionString, migrationsAssembly);
                    break;

                case global::Infrastructure.DatabaseProviders.OracleDatabaseProvider oracleProvider:
                    oracleProvider.ConfigureDbContext<GenerateContext>(services, connectionString, migrationsAssembly);
                    break;

                case global::Infrastructure.DatabaseProviders.PostgreSqlDatabaseProvider postgresProvider:
                    postgresProvider.ConfigureDbContext<GenerateContext>(services, connectionString, migrationsAssembly);
                    break;
